...
public class SearchService {
    ...
    public List<Map<String, Object>> searchByPhonetic(String query) throws IOException {
        ...
        // 1. Извлечение артикулов с помощью DFA-автоматов
        List<ArticleMatch> matches = articleExtractionService.extract(query);

        // 2. Семантический разбор интента и извлечение параметрических фильтров
        ParsedProductQuery parsedQuery = queryParser.parse(query);
        List<Query> filterQueries = buildFilterQueries(parsedQuery);

        List<Query> shouldQueries = new ArrayList<>();

        // 3. Генерация фонетической проекции запроса
        String phoneticQuery = PhoneticUtil.toPhonetic(rawQuery);

        // 4. Динамическое распределение весовых коэффициентов (Сценарии A/B/C)
        if (!matches.isEmpty()) {
            // СЦЕНАРИЙ A/B: Присутствуют явные артикулы (Каталожный поиск)
            for (ArticleMatch match : matches) {
                final String finalNormalized = match.getNormalizedArticle();
                // Точное совпадение артикула в неанализируемом инвертированном индексе
                shouldQueries.add(Query.of(q -> q.term(t -> t.field("allCodes.keyword").value(finalNormalized).boost(2000f))));
                // Префиксное совпадение (поиск по неполному артикулу)
            ...
            // СЦЕНАРИЙ C: Текстовый поиск с параметрическими фильтрами
            shouldQueries.add(Query.of(q -> q.match(m -> m.field("title").query(rawQuery).fuzziness("2").boost(600f))));
            shouldQueries.add(Query.of(q -> q.match(m -> m.field("phonetic").query(phoneticQuery).boost(600f))));
        }
        ...
        List<Map<String, Object>> results = resp.hits().hits()
                ...
                .map(hit -> {
                    ...
                    map.put("id", hit.id());
                    map.put("title", p != null && p.getTitle() != null ? p.getTitle() : "не указано");
                    map.put("manufacturer", p != null && p.getManufacturer() != null ? p.getManufacturer() : "не указано");
                    map.put("productCode", p != null ? p.getProductCode() : null);
                    map.put("externalId", p != null ? p.getExternalId() : null);
